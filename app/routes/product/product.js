...
router.post('/:id/reviews', isAuth, async (req, res) => {
  ...
  if (product) {
    ...
    res.status(404).send({ message: 'Erro ao avaliar' });
  }
});
router.put('/:id', isAuth, isAdmin, async (req, res) => {
  ...
  if (product) {
    ...
    if (updatedProduct) {
      return res
        .status(200)
        .send({ message: 'Produto atualizado', data: updatedProduct });
    }
  }
...
router.post('/post', isAuth, isAdmin, async (req, res) => {
  ...
  return res.status(500).send({ message: ' Erro ao criar o produto.' });
});
